...
// Helper functions for formatting parts of the prompt
...
// Main prompt template functions
export const getChannelSummaryPrompt = (
    mainChannel: Channel,
...
export const getVideoSummaryPrompt = (video: Video, comments: CommentThread[]): string => {
...
export const getLectorSummaryPrompt = (fullAnalysis: string): string => {
    return `
...
export const getUpdateChannelSummaryPrompt = (currentSummary: string, videoInsight: string): string => {
    return `
...
export const getUpdateChangelogPrompt = (oldSummary: string, newSummary: string): string => {
    return `
Jesteś asystentem AI, który pomaga analitykowi zrozumieć, co zmieniło się w raporcie po dodaniu nowego danych. Poniżej znajdują się dwie wersje tego samego raportu: "stara" i "nowa" (zaktualizowana o szczegółową analizę jednego wideo).
...
export const getRefineAnalysisPrompt = (currentAnalysis: string, command: string): string => {
    return `
